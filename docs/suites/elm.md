...
| Shipped shapes          | `credential-verification` `edcGenericFullShapes` / `elmPlainProfileShapes` | re-ships the consumer shape subset (kept in sync by `fetch-elm-artifacts.ts`) so a downstream wallet/verifier need not vendor SHACL; engine stays vendor-free                       |
