# Europass / ELM v3.3 support: VC-agnostic core, all four profiles, SHACL denominator, JAdES seal
...
(`contracts/src/vc-data-model`, the OB 3.0 / CLR 2.0 profiles, and `@conform-ed/credential-verification`).
Decisions below were taken in a grilling session grounded in the live v3.3 artifacts (ELM Browser
3.3.1, `code.europa.eu/.../ELM-support`, the SHACL shapes, and the EU sample credentials).
...
- **LOQ / AMS / PID** — **unsealed plain-dataset** profiles (no VC envelope, no seal), multi-rooted.
  Get contracts + coverage **only**. Their *structural* SHACL validation is deliberately not
  ...
  its already-vendored shapes. *Seal* verification is **not applicable**, not deferred: these
...
The EDC VC envelope is modelled to its **as-shipped shape** (it carries `issuanceDate` *and*
`validFrom` *and* ELM's `issued`), reusing `vc-data-model/v2_0` primitives where they align. We do
...
*language*"), and because an EDC self-declares `ShaclValidator2017`, the denominator is the EU
...
and is viewable via a harness app/route. **PDF export and pixel/background EU-Viewer parity are
deferred** (downstream concerns, no formal render-conformance spec to gate against). `displayParameter`
is still fully modelled + SHACL-validated + coverage-mapped regardless.
