---
title: "Unified TypeScript release, versioning, and tooling standard (emergent / conform-ed / pgxsinkit)"
description: "Architecture decision record ADR-0016."
sidebar:
  order: 16
  badge: { text: "ADR-0016", variant: note }
---
...
incompatible toolchains: bare `format`/`lint` _wrote_ in pgxsinkit but _checked_ in
...
the GitHub-Packages dev channel had just shipped a build that sorted _below_ the latest
...
2. **Trigger → artifact map.**
   ...
   - **Push to `main`** (the manual fast-forward) → a freshly built **dev** package on
     the GitHub Packages npm registry at dist-tag `@dev`. This is _not_ a release.
...
3. **Versioning: the most recent semver tag is the _sole_ source of truth (Solution A).**
   ...
   time, on the runner, where the latest tag _and_ the commit SHA are both known — so no
...
5. **Canonical script vocabulary — check-default.** Bare verbs never mutate; mutation is
   ...
   | `release:npm <tag>` _(publishers only)_ | download GH tarballs → publish to npm | publishes |
   ...
   The script _contract_ (names, semantics, check-default) is what is uniform — not the
