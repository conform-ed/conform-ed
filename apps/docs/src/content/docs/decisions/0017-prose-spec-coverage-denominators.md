---
title: "Hand-authored denominators for prose-only spec families"
description: "Architecture decision record ADR-0017."
sidebar:
  order: 17
  badge: { text: "ADR-0017", variant: note }
---
...
prose-only families the Zod contract is simultaneously the model _and_ the only yardstick, and L2
...
- **Deep Linking `ContentItemSchema`** was a single permissive object covering all five content-item
  ...
  item's `mediaType`/`expiresAt`. The guide requirement (LTI-DL-3) only asserts the _prose_
...
- **What it is.** A small, version-pinned, machine-readable inventory (JSON, the same on-disk shape a
  walker emits) of the family's information model curated _from the prose spec_: each object's
...
- **Shifts, but does not remove, the trust assumption.** A curated denominator is only as complete as
  its curator; an omission in the _inventory_ is still invisible. The mitigation is structural, not
  magical: mandatory per-item `specRef`, ADR-grade review of the curated files, the explicit
  weakest-provenance label, and the drift guard. We are trading an invisible gap for a _cited,
  reviewable_ one — strictly better, not perfect.
