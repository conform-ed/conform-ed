---
title: "Caliper 1.2 — full contract modelling, Sender/Endpoint board, and an event validator"
description: "Architecture decision record ADR-0018."
sidebar:
  order: 18
  badge: { text: "ADR-0018", variant: note }
---

Status: accepted (2026-06-25) — extends [ADR-0013](/decisions/0013-spec-coverage-map/) (coverage map) and [ADR-0017](/decisions/0017-prose-spec-coverage-denominators/) (value-set denominators)
...
3. **Export `validateCaliperEvent` from `@conform-ed/contracts`.** Promote
   ...
   xAPI, where the consumer-side `validateStatementPayload` is genuine LRS _business_ logic.
...
5. **Score the reference-or-inline duality as N/A, not `partial`.** Caliper lets every entity
   ...
   _both_ kinds of slot, so a fully-modelled type showed a band of false `partial`s (its deep fields
   ...
   binding, so this collapses the false band to `yes`; the safeguard restores any field reached _only_
...
6. **Model NavigationEvent's `navigatedFrom` explicitly.** The base event object is `.strict()`, so
   ...
   base with `navigatedFrom` _before_ the refinement, then apply the same rule body. No type
