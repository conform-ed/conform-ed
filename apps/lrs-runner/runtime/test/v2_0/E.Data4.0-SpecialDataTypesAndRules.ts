...
type SpecialDataHelper = {
  OAuthRequest(request: RequestFactory): RequestFactory;
  addAllHeaders(headers?: Record<string, string | undefined>): Record<string, string | undefined>;
  createFromTemplate(template: Array<Record<string, string> | Record<string, JsonValue>>): { statement: StatementV2 };
  generateUUID(): string;
  getEndpointAndAuth(): string;
  getEndpointStatements(): string;
  parse(input: string | object): StatementV2 | Record<string, JsonValue> | string;
  sendRequest(requests: Array<Record<string, JsonValue>>): Promise<void>;
};

const helper = helperImport as unknown as SpecialDataHelper;
const templatingSelection = templatingSelectionImport;
...
function parseMillisecondsFromIso(value: string | undefined): number | null {
  ...
  describe("An Extension can be null, an empty string, objects with nothing in them when using PUT. (Format, Data 4.1, XAPI-00119)", () => {
    ...
    it("statement activity extensions can be empty object", async () => {
      const template = [
        ...
        data = createStatementV2(helper, template);
