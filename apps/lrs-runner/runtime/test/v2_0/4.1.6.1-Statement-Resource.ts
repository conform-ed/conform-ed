...
type XapiResponse<TData = JsonValue> = {
  status: number;
  data: TData;
  headers: XapiHeaders;
};

type XapiRequestsSubset = {
  sendStatementPromise(statement: JsonValue): Promise<XapiResponse>;
  getStatementExact(id: string): Promise<XapiResponse>;
  getStatementExactPromise(id: string): Promise<XapiResponse>;
};

const xapiRequests = xapiRequestsImport as XapiRequestsSubset;
...
function requireObjectWithId(object: StatementV2["object"], source: string): { id: string } {
  if ("id" in object && typeof object.id === "string") {
    return object as { id: string };
  }
  throw new Error(`${source} must include an object.id`);
}

function requireStatementContext(statement: StatementV2, source: string): NonNullable<StatementV2["context"]> {
  return requireV20Context(statement, source);
}

function requireStatementCategory(
  statement: StatementV2,
  source: string,
): ReturnType<typeof requireV20CategoryActivity> {
  return requireV20CategoryActivity(requireStatementContext(statement, source), source);
}

function requireStatementInstructor(statement: StatementV2, source: string) {
...
function requireStatementRegistration(statement: StatementV2, source: string): string {
  const context = requireStatementContext(statement, source);
...
function requireSubStatementCategory(
  statement: StatementV2,
  source: string,
): ReturnType<typeof requireV20CategoryActivity> {
  const subStatement = requireV20SubStatementObject(statement.object, source);
  if (!subStatement.context) {
    throw new Error(`${source} must include substatement.context`);
  }
  return requireV20CategoryActivity(subStatement.context as NonNullable<StatementV2["context"]>, source);
}

type AgentLike = Agent | Group;
type VerbLike = Verb;
...
describe("Statement Resource Requirements (Communication 2.1)", () => {
  ...
  describe('An LRS has a Statement Resource with endpoint "base IRI"+"/statements" (Communication 2.1, XAPI-00139)', function () {
    it('should allow "/statements" POST', async function () {
      ...
      let data = createStatementV2(helper, templates);
  ...
  describe("An LRS cannot modify a Statement, state, or Object in the event it receives a Statement with statementID equal to a Statement in the LRS already. (Communication 2.1.1.s2.b2, XAPI-00142)", function () {
    it('should not update statement with matching "statementId" on PUT', async function () {
      ...
      const statement = parseBody<StatementV2>(helper, res.body);
    ...
    it("should reject a batch of two or more statements where the same ID is used more than once.", async function () {
      let statementOne = helper.buildStatement() as StatementV2;
      ...
      let payload = [statementOne, statementTwo] as JsonValue;
      ...
      try {
        ...
        const responseStatus = typeof httpError.response?.status === "number" ? httpError.response.status : null;
    ...
    it('should include a Last-Modified header which matches the "stored" Timestamp of the statement.', async function () {
      let statement = helper.buildStatement() as StatementV2;
      const postResponse = await xapiRequests.sendStatementPromise(statement as unknown as JsonValue);
      if (!Array.isArray(postResponse.data) || typeof postResponse.data[0] !== "string") {
        throw new Error("Expected statement POST response data to include a statement id at index 0.");
      }
      const storedId = postResponse.data[0];
      const getResponse = await xapiRequests.getStatementExact(storedId);
      const lastModifiedStr = getResponse.headers.get("last-modified");
      ...
      const retrievedStatement = getResponse.data as StatementV2;
      const storedStr = retrievedStatement.stored;
      const stored = Date.parse(storedStr ?? "");
  ...
  describe('An LRS\'s Statement Resource upon processing a successful GET request with a "voidedStatementId" parameter, returns code 200 OK and a single Statement with the corresponding "id".  (Communication 2.1.3.s1, XAPI-00155)', function () {
    ...
    beforeAll(async function () {
      let templates = [{ statement: "{{statements.default}}" }];
      let voided = createStatementV2(helper, templates);
      voided.id = voidedId;
    ...
    beforeAll(async function () {
      ...
      let voiding = createStatementV2(helper, templates);
      requireObjectWithId(voiding.object, "voiding statement").id = voidedId;
  ...
  describe('An LRS\'s Statement Resource upon processing a successful GET request with neither a "statementId" nor a "voidedStatementId" parameter, returns code 200 OK and a StatementResult Object.  (Communication 2.1.3.s1, XAPI-00154)', function () {
    let statement: StatementV2, substatement: StatementV2, stmtTime: number;
    beforeAll(async function () {
      let templates = [
      ...
      requireStatementCategory(statement, "primary statement category").id =
    ...
    beforeAll(async function () {
      ...
      requireSubStatementCategory(substatement, "substatement category").id =
    ...
    it('should return StatementResult using GET with "activity"', async function () {
      let query = helper.getUrlEncoding({ activity: requireObjectWithId(statement.object, "statement activity").id });
      const res = await expectAsync(
        request(helper.getEndpointAndAuth())
          .get(helper.getEndpointStatements() + "?" + query)
          .wait(helper.genDelay(stmtTime, "?" + query, undefined))
    ...
    it('should return StatementResult using GET with "registration"', async function () {
      let query = helper.getUrlEncoding({
        registration: requireStatementRegistration(statement, "statement registration"),
      });
      const res = await expectAsync(
        request(helper.getEndpointAndAuth())
          .get(helper.getEndpointStatements() + "?" + query)
    ...
    it('should return StatementResult using GET with "related_activities"', async function () {
      let query = helper.getUrlEncoding({
        activity: requireStatementCategory(statement, "related activities category").id,
    ...
    it('should return StatementResult using GET with "related_agents"', async function () {
      let query = helper.getUrlEncoding({
        agent: requireStatementInstructor(statement, "related agents instructor"),
        related_agents: true,
      });
      const res = await expectAsync(
        request(helper.getEndpointAndAuth())
  ...
  describe('An LRS\'s Statement Resource can process a GET request with "related_activities" as a parameter  (**Implicit**, Communication 2.1.3.s1.table1.row7)', function () {
    let statement: StatementV2, stmtTime: number;
    ...
    beforeAll(async function () {
      ...
      let data = helper.createFromTemplate(templates) as { statement: StatementV2 };
      statement = data.statement;
      requireStatementCategory(statement, "related activities statement category").id =
        "http://www.example.com/test/array/statements/pri";
      stmtTime = Date.now();
    ...
    it('should process using GET with "related_activities"', async function () {
      let query = helper.getUrlEncoding({
        activity: requireStatementCategory(statement, "related activities query category").id,
        related_activities: true,
      });
      await expectAsync(
        request(helper.getEndpointAndAuth())
  ...
  describe('An LRS\'s Statement Resource can process a GET request with "related_agents" as a parameter  (**Implicit**, Communication 2.1.3.s1.table1.row8, XAPI-00176)', function () {
    ...
    beforeAll(async function () {
      ...
      let data = helper.createFromTemplate(templates) as { statement: StatementV2 };
      statement = data.statement;
      requireStatementCategory(statement, "related agents statement category").id =
        "http://www.example.com/test/array/statements/pri";
      stmtTime = Date.now();
    ...
    it('should process using GET with "related_agents"', async function () {
      let query = helper.getUrlEncoding({
        agent: requireStatementInstructor(statement, "related agents query instructor"),
        related_agents: true,
      });
      await expectAsync(
        request(helper.getEndpointAndAuth())
  ...
  describe('If the "Accept-Language" header is present as part of the GET request to the Statement API and the "format" parameter is set to "canonical", the LRS MUST apply this data to choose the matching language in the response. (Communication 2.1.3.s1.table1.row11, XAPI-00172)', function () {
    ...
    beforeAll(async function () {
      ...
      statement = createStatementV2(helper, templates);
      requireStatementCategory(statement, "accept-language fixture category").id =
        "http://www.example.com/test/array/statements/pri";
      stmtTime = Date.now();
      const res = await expectAsync(
        request(helper.getEndpointAndAuth())
    ...
    it("should apply this data to choose the matching language in the response", async function () {
      ...
      const res = await expectAsync(
        request(helper.getEndpointAndAuth())
          .get(helper.getEndpointStatements() + "?" + query)
          .wait(helper.genDelay(stmtTime, undefined, statementID))
          .headers(helper.addAllHeaders({ "Accept-Language": "en-GB" })),
        200,
      );

      const statement = parseBody<StatementV2>(helper, res.body);
      const category = requireStatementCategory(statement, "canonical response category");
    ...
    it("should NOT apply this data to choose the matching language in the response when format is not set ", async function () {
      ...
      const statement = parseBody<StatementV2>(helper, res.body);
      const category = requireStatementCategory(statement, "exact response category");
  ...
  describe('An LRS\'s Statement Resource can process a GET request with "format" as a parameter  (**Implicit**, Communication 2.1.3.s1.table1.row12)', function () {
    ...
    beforeAll(async function () {
      ...
      let data = createStatementV2(helper, templates);
      const dataSubStatement = requireV20SubStatementObject(data.object, "format fixture substatement");
      agent = data.actor;
      agent.mbox = "mailto:agent" + helper.generateUUID() + "@adlnet.gov";
      verb1 = data.verb;
      group = dataSubStatement.actor;
      group.mbox = "mailto:group" + helper.generateUUID() + "@adlnet.gov";
      verb2 = dataSubStatement.verb;
      requireObjectWithId(dataSubStatement.object as StatementV2["object"], "format fixture activity").id =
        "http://www.example.com/unicode/" + helper.generateUUID();
      activity = dataSubStatement.object as ActivityLike;
      stmtTime = Date.now();
      const res = await expectAsync(
        request(helper.getEndpointAndAuth())
          .post(helper.getEndpointStatements())
    ...
    it('should process using GET with "format" absent (XAPI-00168)', async function () {
      ...
      stmts.forEach(function (stmt: StatementV2) {
        if (stmt.id === id) {
          const stmtSubStatement = requireV20SubStatementObject(stmt.object, "format absent substatement");
          expect(stmt.actor).toEqual(agent);
          expect(stmt.verb).toEqual(verb1);
          expect(stmtSubStatement.actor).toEqual(group);
          expect(stmtSubStatement.object).toEqual(activity);
    ...
    it('should process using GET with "format" exact (XAPI-00170)', async function () {
      ...
      stmts.forEach(function (stmt: StatementV2) {
        if (stmt.id === id) {
          const stmtSubStatement = requireV20SubStatementObject(stmt.object, "format exact substatement");
          expect(stmt.actor).toEqual(agent);
          expect(stmt.verb).toEqual(verb1);
          expect(stmtSubStatement.actor).toEqual(group);
          expect(stmtSubStatement.verb).toEqual(verb2);
    ...
    it('should process using GET with "format" ids (XAPI-00171)', async function () {
      ...
      stmts.forEach(function (stmt: StatementV2) {
        if (stmt.id === id) {
          const stmtSubStatement = requireV20SubStatementObject(stmt.object, "format ids substatement");
          const stmtSubActivity = requireObjectWithId(
            stmtSubStatement.object as StatementV2["object"],
            "format ids subactivity",
          );
          ...
          expect(Object.keys(stmtSubStatement.verb).length).toEqual(1);
          /** Re-adding these as it's once again a requirement for 2.0 */
  ...
  describe('An LRS\'s Statement Resource can process a GET request with "attachments" as a parameter  (**Implicit**, Communication 2.1.3.s1.table1.row13, XAPI-00167)', function () {
    ...
    beforeAll(async function () {
      ...
      const postedIds = parseBody<string[]>(helper, res.body);
      stmtId = postedIds[0] ?? "";
    });
    ...
    it('should return multipart response format StatementResult using GET with "attachments" parameter as true', async function () {
      ...
      const results = parseBody<StatementResultV2>(helper, firstPart.body);
      expect(results).toHaveProperty("statements");
    });
    ...
    it('should process using GET with "attachments"', async function () {
      ...
      let result = parseBody<Record<string, unknown>>(helper, c);
  ...
  describe('An LRS\'s "X-Experience-API-Consistent-Through" header is an ISO 8601 combined date and time (Type, Communication 2.1.3.s2.b5).', function () {
    ...
    beforeAll(async function () {
      ...
      statement = createStatementV2(helper, templates);
      requireStatementCategory(statement, "consistent-through statement category").id =
        "http://www.example.com/test/array/statements/pri";
    ...
    it('should return "X-Experience-API-Consistent-Through" using GET with "related_activities"', async function () {
      let query = helper.getUrlEncoding({
        activity: requireStatementCategory(statement, "consistent-through related activities category").id,
        related_activities: true,
      });
      const res = await expectAsync(
        request(helper.getEndpointAndAuth())
    ...
    it('should return "X-Experience-API-Consistent-Through" using GET with "related_agents"', async function () {
      let query = helper.getUrlEncoding({
        agent: requireStatementInstructor(statement, "consistent-through related agents instructor"),
        related_agents: true,
      });
      const res = await expectAsync(
        request(helper.getEndpointAndAuth())
  ...
  describe("An LRS's Statement Resource, upon processing a successful GET request wishing to return a Voided Statement still returns Statements which target it (Communication 2.1.4.s1.b2, XAPI-00162)", function () {
    ...
    beforeAll(async function () {
      ...
      let voided = createStatementV2(helper, voidedTemplates);
      voided.id = voidedId;
      voided.verb.id = verb;
    ...
    beforeAll(async function () {
      ...
      let voiding = createStatementV2(helper, voidingTemplates);
      ...
      await expectAsync(
        request(helper.getEndpointAndAuth())
          .post(helper.getEndpointStatements())
          .wait(helper.genDelay(Date.parse(sinceVoidingTime), "?statementId=" + voidedId, voidedId))
          .headers(helper.addAllHeaders({}))
          .json(voiding),
        200,
      );
    ...
    beforeAll(async function () {
      ...
      let statementRef = createStatementV2(helper, statementRefTemplates);
      statementRef.id = statementRefId;
      requireObjectWithId(statementRef.object, "statement-ref object").id = voidedId;
    ...
    it('should only return statements stored after designated "since" timestamp when using "since" parameter', async function () {
      ...
      results.statements.forEach(function (stmt: StatementV2) {
        ids.push(stmt.id);
      });
      // console.log(ids);
  ...
  describe('The Statements within the "statements" property will correspond to the filtering criterion sent in with the GET request (Communication 2.1.3.s1, XAPI-00164)', function () {
    ...
    beforeAll(async function () {
      ...
      requireStatementCategory(statement, "filtering statement category").id += helper.generateUUID();
      statement.verb.id += helper.generateUUID();
      statement.actor.mbox = "mailto:" + helper.generateUUID() + "@adlnet.gov";
      requireStatementContext(statement, "filtering statement context").registration = helper.generateUUID();
      requireStatementInstructor(statement, "filtering statement instructor").mbox =
        "mailto:" + helper.generateUUID() + "@adlnet.gov";
      requireObjectWithId(statement.object, "filtering statement object").id += helper.generateUUID();

      requireStatementCategory(statement, "filtering statement category").id =
        "http://www.example.com/test/array/statements/pri";
    ...
    beforeAll(async function () {
      ...
      let data = helper.createFromTemplate(templates) as { statement: StatementV2 };
      substatement = data.statement;
      const sub = requireV20SubStatementObject(substatement.object, "filtering substatement object");

      //randomize data to prevent old results from breaking assertion logic
      substatement.verb.id += helper.generateUUID();
      substatement.actor.mbox = "mailto:" + helper.generateUUID() + "@adlnet.gov";

      sub.verb.id += helper.generateUUID();
      sub.actor.mbox = "mailto:" + helper.generateUUID() + "@adlnet.gov";
      requireObjectWithId(sub.object as StatementV2["object"], "filtering substatement activity").id +=
        helper.generateUUID();

      requireSubStatementCategory(substatement, "filtering substatement category").id =
        "http://www.example.com/test/array/statements/sub";
      stmtTime = Date.now();
      await expectAsync(
        request(helper.getEndpointAndAuth())
    ...
    it('should return StatementResult with statements as array using GET with "activity"', async function () {
      let query = helper.getUrlEncoding({
        activity: requireObjectWithId(statement.object, "filtering query activity").id,
      });
      const res = await expectAsync(
        request(helper.getEndpointAndAuth())
          .get(helper.getEndpointStatements() + "?" + query)
      ...
      expect(
        statements.every(
          (statementItem) =>
            statementItem.object?.id === requireObjectWithId(statement.object, "filtering compare activity").id,
        ),
      ).toBe(true);
    });
    ...
    it('should return StatementResult with statements as array using GET with "registration"', async function () {
      let query = helper.getUrlEncoding({
        registration: requireStatementRegistration(statement, "filtering query registration"),
      });
      const res = await expectAsync(
        request(helper.getEndpointAndAuth())
          .get(helper.getEndpointStatements() + "?" + query)
      ...
      expect(
        statements.every(
          (statementItem) =>
            ...
            requireStatementRegistration(statement, "filtering compare registration"),
        ),
      ).toBe(true);
    });
    ...
    it('should return StatementResult with statements as array using GET with "related_activities"', async function () {
      let query = helper.getUrlEncoding({
        activity: requireStatementCategory(statement, "filtering query category").id,
        related_activities: true,
      });
      const res = await expectAsync(
        request(helper.getEndpointAndAuth())
      ...
      expect(Array.isArray(result.statements)).toBe(true);
      expect(
        result.statements.every((statementItem: StatementV2) =>
          helper.deepSearchObject(
            statementItem,
            requireStatementCategory(statement, "filtering deep-search category").id,
          ),
        ),
      ).toBe(true);
    });
    ...
    it('should return StatementResult with statements as array using GET with "related_agents"', async function () {
      let query = helper.getUrlEncoding({
        agent: requireStatementInstructor(statement, "filtering query instructor"),
        related_agents: true,
      });
      const res = await expectAsync(
        request(helper.getEndpointAndAuth())
      ...
      expect(
        result.statements.every((statementItem: StatementV2) =>
          helper.deepSearchObject(
            statementItem,
            requireStatementInstructor(statement, "filtering deep-search instructor").mbox,
          ),
        ),
      ).toBe(true);
    });
    ...
    it('should return StatementResult with statements as array using GET with "since"', async function () {
      ...
      expect(
        result.statements.every(
          (statementItem: StatementV2) => new Date(statementItem.stored ?? 0) >= new Date("2012-06-01T19:09:13.245Z"),
        ),
      ).toBe(true);
    });
    ...
    it('should return StatementResult with statements as array using GET with "ascending"', async function () {
      ...
      expect(result).toHaveProperty("statements");
      expect(Array.isArray(result.statements)).toBe(true);
      const ascending = result.statements.every((statementItem, index, statements) => {
        if (index === 0) {
          return true;
        }
  ...
  describe("An LRS's Statement Resource rejects with error code 400 a GET request with additional properties than extensions in the locations where extensions are allowed", function () {
    it("should fail when using property not defined in specification", async function () {
      let statement = helper.buildStatement() as StatementV2 & { dummy?: string };
      statement.dummy = "dummy";
      try {
        const res = await xapiRequests.sendStatementPromise(statement as unknown as JsonValue);
        expect(res.status).toEqual(400);
      } catch (err) {
        const httpError = err as { response?: { status?: number } };
        const responseStatus = typeof httpError.response?.status === "number" ? httpError.response.status : null;
  ...
  describe('The LRS shall set the "timestamp" property to the value of the "stored" property if not provided.', function () {
    it('should set timestamp property to equal "stored" value if retrieved statement does not have its own timestamp', async function () {
      ...
      (statement as unknown as { timestamp: string | null }).timestamp = null;
      statement.id = id;

      await xapiRequests.sendStatementPromise(statement as unknown as JsonValue);
      const res = await xapiRequests.getStatementExactPromise(id);

      const statementFromLRS = res.data as StatementV2;
